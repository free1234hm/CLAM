...
public class DataSetCore {
	...
	public Integer[] all2this, this2all;
	/**
	...
	public void normalization(List<String> list) {
		controlnorm = new double[numrows][numcols];
			for(int i=0;i<numrows;i++){
				if(!list.contains(genenames[i])) {
	...
	public void filtergenes(List<String> list, int[][] measuredtable, int column) {
		Integer[] all2this = new Integer[list.size()];
		Integer[] this2all = new Integer[numrows];
		for(int i=0;i<list.size();i++) {
			for(int j=0;j<numrows;j++) {
				if(list.get(i).equals(genenames[j])) {
					all2this[i] = j;
					this2all[j] = i;
					measuredtable[i][column] = 1;
					break;
				}
			}
		}
		this.all2this = all2this;
		this.this2all = this2all;
	}
	...
	public DataSetCore logratio2() {
		...
		return new DataSetCore(szInputFile, genespotcontroldata,
				...
				controldata, controlnorm, pmacontrol, probenames, genenames, dthresholdvalue, 
	...
	public DataSetCore filtergenesgeneral(boolean[] keepgene, int nkeep, boolean bstore) {
		...
		return new DataSetCore(szInputFile, filtergenespotcontroldata,
				...
				filterpmacontrol, filterprobenames, filtergenenames,
